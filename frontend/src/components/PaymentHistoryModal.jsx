...
const PaymentHistoryModal = ({ memberId, memberName, memberFolio, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-2 sm:p-4">
      <div className="bg-white rounded-xl shadow-2xl max-w-6xl w-full max-h-[95vh] sm:max-h-[90vh] overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-200 bg-gradient-to-r from-primary-600 to-primary-700">
          <div className="flex items-center space-x-2 sm:space-x-3 min-w-0">
            <div className="p-1.5 sm:p-2 bg-white bg-opacity-20 rounded-lg flex-shrink-0">
              <CreditCard className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
            </div>
            <div className="min-w-0">
              <h2 className="text-lg sm:text-xl font-bold text-white">Payment History</h2>
              <p className="text-xs sm:text-sm text-primary-100 truncate">{memberName} • {memberFolio}</p>
            </div>
          </div>
          <button
            onClick={onClose}
              ...
              {data?.summary && (
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4 mb-4 sm:mb-6">
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 sm:p-4">
                    <p className="text-xs sm:text-sm text-blue-600 font-medium mb-1">Total Periods</p>
                    <p className="text-xl sm:text-2xl font-bold text-blue-700">
                      {data.summary.totalPeriods}
                    </p>
                  </div>
                  <div className="bg-green-50 border border-green-200 rounded-lg p-3 sm:p-4">
                    <p className="text-xs sm:text-sm text-green-600 font-medium mb-1">Paid Periods</p>
                    <p className="text-xl sm:text-2xl font-bold text-green-700">
                      {data.summary.paidPeriods}
                    </p>
                  </div>
                  <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 sm:p-4">
                    <p className="text-xs sm:text-sm text-amber-600 font-medium mb-1">Unpaid Periods</p>
                    <p className="text-xl sm:text-2xl font-bold text-amber-700">
                      {data.summary.unpaidPeriods}
                    </p>
                  </div>
                  <div className="bg-purple-50 border border-purple-200 rounded-lg p-3 sm:p-4">
                    <p className="text-xs sm:text-sm text-purple-600 font-medium mb-1">Total Revenue</p>
        ...
        {/* Footer */}
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-0 p-3 sm:p-6 border-t border-gray-200 bg-gray-50">
          <div className="text-xs sm:text-sm text-gray-600">
            Member ID: {data?.member?.id} • Class: {data?.member?.member_class || 'N/A'}
          </div>
          <button
            onClick={onClose}
            className="px-4 sm:px-6 py-2 text-sm sm:text-base bg-primary-600 text-white rounded-lg font-medium hover:bg-primary-700 transition-colors"
          >
            Close
          </button>
        </div>
